    ...
    def load_data(self, csv_path: Optional[str] = None, url: Optional[str] = None) -> Tuple[Optional[pd.DataFrame], Optional[float], Optional[float], bool, str]:
        ...
        if url:
            try:
                ...
                df = pd.read_html(html)[0]   # don't hardcode id, table id changes
                data_source = f"URL: {url}"
                st.success(f"✅ Data loaded from URL using custom headers")
...
def main():
    ...
    csv_path = st.sidebar.text_input(
        "CSV File Path", 
        value="premier_league_stats.csv",
        help="Path to your CSV file (e.g., 'data.csv' or '/path/to/data.csv')"
    )
    
    # Load data with caching - CSV priority, URL fallback
    url = 'https://fbref.com/en/comps/9/Premier-League-Stats'
    df, league_avg_home_goals, league_avg_away_goals, success, data_source = load_football_data(csv_path, url)
