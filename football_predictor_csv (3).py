...
class FootballPredictor:
    def __init__(self):
        ...
        self.league_avg_home_goals: Optional[float] = None
        self.league_avg_away_goals: Optional[float] = None

    def load_data(self, csv_path: Optional[str] = None, url: Optional[str] = None) -> Tuple[Optional[pd.DataFrame], Optional[float], Optional[float], bool, str]:
        """Load data from CSV first, then URL as fallback"""
        data_source = "Unknown"
            ...
            try:
                ...
                for encoding in encodings:
                    try:
                        ...
                        data_source = f"CSV: {csv_path} (encoding: {encoding})"
                        st.success(f"✅ Data loaded from CSV with {encoding} encoding")
                        break
                ...
                league_avg_home_goals, league_avg_away_goals = self._calculate_league_averages(df)
                
                return df, league_avg_home_goals, league_avg_away_goals, True, data_source
                
            except FileNotFoundError:
                st.warning(f"⚠️ CSV file not found: {csv_path}. Trying URL...")
            except Exception as e:
                st.warning(f"⚠️ Error loading CSV: {e}. Trying URL...")
        ...
        if url:
            try:
                ...
                st.success(f"✅ Data loaded from URL")
        ...
        st.warning("⚠️ Using sample data. Please provide valid CSV or check URL.")
    ...
    def _process_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """Process and clean dataframe regardless of source"""
        # Handle multi-level columns from URL scrape
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = ['_'.join(col).strip() for col in df.columns.values]
        
        # Find the team column (could be 'Squad', 'Team', or similar)
        team_col = None
        ...
        if team_col is None:
            # Use second column as team name (common in scraped data)
        ...
        numeric_cols = [col for col in df.columns if any(x in col for x in ['MP', 'GF', 'GA'])]
        for col in numeric_cols:
            df[col] = pd.to_numeric(df[col], errors='coerce')
        
        return df
    
    def _calculate_league_averages(self, df: pd.DataFrame) -> Tuple[float, float]:
    ...
    def dixon_coles_adjustment(self, home_goals: int, away_goals: int, rho: float = 0.1) -> float:
    ...
    def enhanced_poisson_prediction(
        self, 
        home_team: str, 
        away_team: str, 
        df: pd.DataFrame, 
        ...
        injury_handicap_home: float = 0.0, 
        injury_handicap_away: float = 0.0, 
...
def load_football_data(csv_path: str, url: str) -> Tuple[Optional[pd.DataFrame], Optional[float], Optional[float], bool, str]:
    """Cached data loading function"""
    predictor = FootballPredictor()
    return predictor.load_data(csv_path=csv_path, url=url)

def create_outcome_chart(result: Dict[str, Any], home_team: str, away_team: str) -> go.Figure:
    ...
    fig.update_layout(
        title=dict(
            text=f'Match Outcome Probabilities<br>{home_team} vs {away_team}',
            x=0.5,
            xanchor='center'
        ),
...
def create_score_matrix(result: Dict[str, Any]) -> go.Figure:
...
def create_special_markets_chart(result: Dict[str, Any]) -> go.Figure:
...
def create_expected_goals_chart(result: Dict[str, Any], home_team: str, away_team: str) -> go.Figure:
...
def main():
    ...
    csv_url = st.sidebar.text_input(
        "CSV URL", 
        value="https://raw.githubusercontent.com/Bademosi95/PL_DATA1/main/premier_league_stats.csv",
        help="URL to your CSV file (GitHub raw URL or local path)"
    )
    
    # Load data with caching - CSV priority, URL fallback
    fallback_url = 'https://fbref.com/en/comps/9/Premier-League-Stats'
    df, league_avg_home_goals, league_avg_away_goals, success, data_source = load_football_data(csv_url, fallback_url)
    
    if not success or df is None:
        st.error("Failed to load data from all sources. Please check your CSV file or internet connection.")
        st.stop()
    
    # Display data source
    st.sidebar.info(f"📊 Data loaded from: {data_source}")
